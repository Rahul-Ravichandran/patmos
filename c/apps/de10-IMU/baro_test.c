...
//Writes to i2c, returns -1 if there was an error, 0 if succeded
int i2c_write_short(unsigned char chipaddress, unsigned char regaddress){
  I2C = ((((unsigned int) regaddress & 0x000000FF) << 8) | (((unsigned int) chipaddress & 0x0000007F) << 1)) & 0xFFFFFFFE;
  if ((I2C & 0x00000100) != 0)
  {
    return -1;
  }else{
...
{
  // printf("random = %X",i2c_read(MS5611_ADDR, 0x48));
  i2c_write_short(MS5611_ADDR, 0x1E);
  millis(5);
  i2c_write_short(MS5611_ADDR, 0x48);
  millis(10);
  
  data[0] = i2c_read(MS5611_ADDR, 0x00);
  data[1] = i2c_read(MS5611_ADDR, 0x00+1);
  data[2] = i2c_read(MS5611_ADDR, 0x00+2);
  
  unsigned long ptemp = ((data[0] * 65536.0) + (data[1] * 256.0) + data[2]);
  
  // printf("data[0] = %X  ",data[0]);
  // printf("data[1] = %X  ",data[1]);
  // printf("data[2] = %X",data[2]);

  i2c_write_short(MS5611_ADDR, 0x58);
  millis(10);
...
//   for(int i=0;i<100000;i++) {                                           //Stay in this loop until the data variable data holds a q.
...
//         raw_temperature = i2c_read(MS5611_address, 0x00) << 16 | i2c_read(MS5611_address, 0x00 + 1) << 8 | i2c_read(MS5611_address, 0x00 + 2);
//       }
//       else {
//         //Get pressure data from MS-5611
//         raw_pressure = i2c_read(MS5611_address, 0x00) << 16 | i2c_read(MS5611_address, 0x00 + 1) << 8 | i2c_read(MS5611_address, 0x00 + 2);
//       }
...
int main(int argc, char **argv)
{
  ...
  for (int i = 0; i < 6; i++) {
      data[0] = i2c_read(MS5611_address, 0xA2 + i*2);
      data[1] = i2c_read(MS5611_address, 0xA2 + i*2 +1);                    //Start communication with the MPU-6050.
